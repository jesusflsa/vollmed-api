...
public class MedicoController {
    ...
    @GetMapping
    public ResponseEntity<Page<DatosListadoMedico>> listadoMedicos(@PageableDefault(size = 2) Pageable page) {
        return ResponseEntity.ok(medicoRepository.findAllByActivoTrue(page)
...
//  }
    ...
    @GetMapping("/{id}")
    @Transactional
    public ResponseEntity<DatosRespuestaMedico> mostrarMedico(@PathVariable Long id) {
        Medico medico = medicoRepository.getReferenceById(id);
        return ResponseEntity.ok(new DatosRespuestaMedico(medico));
    }
